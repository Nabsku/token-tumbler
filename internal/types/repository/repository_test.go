...
func TestConfig_Validate(t *testing.T) {
	...
	t.Run("accepts valid k8s secret store config", func(t *testing.T) {
		repo := validRepositoryConfig()
		repo.SecretStore = "k8s"
		repo.K8sNamespace = gitlab.Ptr("default")
		repo.K8sSecretName = gitlab.Ptr("my-secret")
		repo.K8sSecretKey = gitlab.Ptr("token")
		repo.VaultPath = nil
		repo.VaultKey = nil
		repo.Mount = nil
	...
	t.Run("rejects k8s secret store without namespace", func(t *testing.T) {
		repo := validRepositoryConfig()
		repo.SecretStore = "k8s"
		repo.K8sSecretName = gitlab.Ptr("my-secret")
		repo.K8sSecretKey = gitlab.Ptr("token")
		...
		require.ErrorIs(t, err, ErrInvalidRepositoryConfig)
		assert.Contains(t, err.Error(), "k8sNamespace is required")
	})

	t.Run("rejects k8s secret store without secret name", func(t *testing.T) {
		repo := validRepositoryConfig()
		repo.SecretStore = "k8s"
		repo.K8sNamespace = gitlab.Ptr("default")
		repo.K8sSecretKey = gitlab.Ptr("token")
		...
		require.ErrorIs(t, err, ErrInvalidRepositoryConfig)
		assert.Contains(t, err.Error(), "k8sSecretName is required")
	})

	t.Run("rejects k8s secret store without secret key", func(t *testing.T) {
		repo := validRepositoryConfig()
		repo.SecretStore = "k8s"
		repo.K8sNamespace = gitlab.Ptr("default")
		repo.K8sSecretName = gitlab.Ptr("my-secret")
		...
		require.ErrorIs(t, err, ErrInvalidRepositoryConfig)
		assert.Contains(t, err.Error(), "k8sSecretKey is required")
	})
...
func TestConfig_UsesVaultToken(t *testing.T) {
	assert.False(t, (&Config{Repos: []Repository{{SecretStore: "none"}}}).UsesVaultToken())
	assert.False(t, (&Config{Repos: []Repository{{SecretStore: "vault"}}}).UsesVaultToken())
	assert.False(t, (&Config{Repos: []Repository{{SecretStore: "vault", VaultAuthMethod: gitlab.Ptr("approle")}}}).UsesVaultToken())
	assert.False(t, (&Config{Repos: []Repository{{SecretStore: "vault", VaultAuthMethod: gitlab.Ptr("kubernetes")}}}).UsesVaultToken())
	assert.False(t, (&Config{Repos: []Repository{{SecretStore: "vault", VaultAuthMethod: gitlab.Ptr("aws")}}}).UsesVaultToken())
	assert.True(t, (&Config{Repos: []Repository{{SecretStore: "vault", VaultAuthMethod: gitlab.Ptr("token")}}}).UsesVaultToken())
}
